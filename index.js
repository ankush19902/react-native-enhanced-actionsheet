...
export default class EnhancedActionSheet extends Component {
        ...
        <View style={styles.backgroundLayer}>
          <View style={styles.container}>
            <View style={styles.actionsheetContainer}>
              <View
                style={[styles.titleContainer, this._styleTitleContainer()]}
              >
                <Text style={[styles.title, this._styleTitle()]}>
                  {title ? title : "Select"}
                </Text>
              </View>
              <ScrollView showsVerticalScrollIndicator={false}>
                {data.map((e, i) => this._renderOption(e, i, dataLength))}
              </ScrollView>
            </View>
            <TouchableOpacity
              onPress={onCancelPress}
              activeOpacity={1}
              style={[styles.cancelContainer, this._styleCancelContainer()]}
            >
              <Text style={[styles.cancel, this._styleCancelText()]}>
  ...
  _renderOption = (e, i, dataLength) => (
    <TouchableOpacity
      onPress={() =>
        this.props.onOptionPress ? this.props.onOptionPress(e) : {}
      }
      key={i}
      activeOpacity={1}
      style={[
